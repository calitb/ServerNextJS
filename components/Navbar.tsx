...
export default function Navbar() {
  ...
  const activeRoute = 'bg-gray-900 text-white ';
    ...
    <nav className="bg-gray-800">
        ...
        <div className="relative flex items-center justify-between h-16">
          <div className="absolute inset-y-0 left-0 flex items-center sm:hidden">
            <button
              ...
              aria-expanded="false"
            >
              <svg onClick={openMenu} id="mobile-menu-icon-open" className="block h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
          ...
          <div className="flex-1 flex items-center justify-center sm:items-stretch sm:justify-start">
            <div className="flex-shrink-0 flex items-center">
              <Link href="/">
                <a className="text-lg text-white">calitb.dev</a>
              </Link>
            </div>
              ...
              <div className="flex space-x-4">
                {routes.map(({ name, url, active }) => (
                  <Link key={name} href={url}>
                    <a className={`${active ? activeRoute : inactiveRoute} px-3 py-2 rounded-md text-sm font-medium`}>{name}</a>
                  </Link>
                ))}
              </div>
            </div>
      ...
      <div id="mobile-menu" className="hidden sm:hidden">
        <div className="px-2 pt-2 pb-3 space-y-1">
          {routes.map(({ name, url, active }) => (
            <Link key={name} href={url}>
              <a className={`${active ? activeRoute : inactiveRoute}  block px-3 py-2 rounded-md text-base font-medium`}>{name}</a>
            </Link>
          ))}
        </div>
      </div>
